...
- **Top nav:** Cockpit · Holdings · HELOC · Net Worth · Watchlist · Settings. Right side: portfolio scope label (v0.1 always "Individual"), cloud-privacy indicator (☁ Public / 🔒 Local).
...
- Colorblind-safe G/L: shape (●/▲/■) + color.
...
- KPI row: HELOC limit · Drawn · Available · Rate · Monthly Interest · Leverage Ratio. ("Tax-ded. YTD" removed.)
- **Drawdown ledger:** Date · Amount · Purpose (free-text). No tax-deductible column.
- **What-if panel:** Slider for additional draw; shows new balance, new ratio, new monthly interest, safety zone.
...
- **Asset/Liability ledger** (left): Portfolio (auto) · Cash (manual) · Other assets (manual) · Property (manual with slider/input) · Mortgage (manual with slider) · HELOC (auto from HELOC screen) · Other debt (manual).
...
- **HELOC:** Rate (% annual) — consumed by interest and what-if calcs
